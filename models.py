...
class member_application_payments(models.Model):
        _name = 'sale.investor.application.payments'

        member_application_id = fields.Many2one('sale.investor.registration')
        amount = fields.Float()
        date = fields.Date()
        ...
        def auto_post_receipts(self):
                receipts = self.env['sale.investor.application.payments'].search([('processed','=',False),('flagged','=',False)])
